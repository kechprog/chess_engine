...
/// Multithreaded MCTS search using root parallelization
///
/// Runs multiple independent MCTS trees in parallel and merges their results.
/// Each thread gets iterations/num_threads iterations to run.
///
/// # Arguments
///
/// * `pos` - The position to search from
/// * `color` - The color to move
/// * `iterations` - Total number of iterations to run across all threads
/// * `num_threads` - Number of threads to use (if None, uses num_cpus)
///
/// # Returns
///
...
pub struct MCTSStats {
    ...
    pub per_thread_visits: Vec<u32>,
}

impl MCTSStats {
    /// Create stats from single-threaded search
    fn from_single_tree(root_visits: u32, num_children: usize, best_move_visits: u32) -> Self {
        Self {
            root_visits,
            num_children,
            best_move_visits,
            num_threads: 1,
            per_thread_visits: vec![root_visits],
        }
    }

    /// Create stats from multi-threaded search
    fn from_multi_tree(root_visits: u32, num_children: usize, best_move_visits: u32,
                       num_threads: usize, per_thread_visits: Vec<u32>) -> Self {
        Self {
            root_visits,
            num_children,
            best_move_visits,
